...
(define (extend-env-rec proc-name args body env)
  (list (list proc-name args body) env))
(define (apply-env env var)
  (cond
   ...
   ((eqv? var (car (car env)))
    (if (= 2 (length (car env)))
      (cadr (car env))
      (proc-val (procedure (cadr (car env)) (caddr (car env)) env))))
   (else
  ...
  (cases expression exp
    ...
    (call-exp (rator rand)
      (apply-procedure
       ...
       (value-of rand env)))
    (letrec-exp (proc-name arg proc-body let-body)
       (value-of let-body (extend-env-rec proc-name arg proc-body env)))
    ))
